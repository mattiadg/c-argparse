 ...
 */
#define FAIL() printf("\nfailure in %s() line %d\n", __func__, __LINE__)
#define FAIL_EXPECTED(value, expected) do { printf("\nexpected %s but found %s", expected, value); FAIL(); } while (0);
#define _assert(test) do { if (!(test)) { FAIL(); return 1; } } while(0)
#define _assert_str_eq(value, expected, N) do { if (strncmp((value), (expected), (N))) { FAIL_EXPECTED(value, expected); return 1; } } while(0)
